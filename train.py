...
def main(args):
    ...
    train_loader, val_loader = create_dataloader(args, train_data_list, val_data_list, class_to_idx)
...
def create_dataloader(args, train_data_list, val_data_list, class_to_idx):
    from src.helper_functions.augmentations import get_aug_trnsform, get_eval_trnsform
    trans_aug = get_eval_trnsform()
    trans_eval = get_eval_trnsform()
    dataset_train = BirdImageLoader(args.data_path, train_data_list, class_to_idx, transform=trans_aug)
    dataset_val = BirdImageLoader(args.data_path, val_data_list, class_to_idx, transform=trans_eval)
...
def train(args, model, train_loader, val_loader,writer, device):
    ...
    for epoch in range(args.epochs):
        ...
        print('-' * 10)
        train_loss, train_acc_top1, train_acc_top5 = pass_epoch(model, train_loader, model_optimizer, loss_fn,scaler,device,  'Train')  
        with torch.no_grad():
            val_loss, val_acc_top1, val_acc_top5 = pass_epoch(model, val_loader,model_optimizer,loss_fn,scaler, device, 'Eval') 
        ...
        if (val_loss <= min_val_loss):
            ...
            torch.save(model, '{}/checkpoint_{:04d}.pth.tar'.format(args.output_foloder, epoch + 1))
        else:
            stop += 1
            if (stop > 5):
